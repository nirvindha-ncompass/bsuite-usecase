#!/usr/bin/env python3
"""
PostgreSQL Transaction Data Generator (Docker Version - 10M Rows)
=================================================================
Seeds 10M rows into the Docker PostgreSQL instance on port 5433.
Matches the scale of the local environment.
"""
...
# =============================================================================
# DATA GENERATION CONFIGURATION (10M Rows)
# =============================================================================
NUM_CUSTOMERS = 1_000_000      # 1M customers
NUM_MERCHANTS = 100_000        # 100K merchants
NUM_TRANSACTIONS = 10_000_000  # 10M transactions

BATCH_SIZE = 100_000           # Larger batch size for speed
...
def bulk_insert_with_copy(conn, table_name: str, columns: List[str], generator_func, total_rows: int, **kwargs):
    ...
    while rows_inserted < total_rows:
        ...
        if rows_inserted % 1_000_000 == 0 or rows_inserted == total_rows:
             ...
             print(f"   ⏳ Progress: {rows_inserted:,}/{total_rows:,} ({rate:,.0f} rows/s)")
...
def create_indexes_and_constraints(conn):
    ...
    for idx_name, table, column in indexes:
        ...
        print(f"   Created {idx_name} in {time.time() - start:.2f}s")
        
    start = time.time()
    cursor.execute("CREATE INDEX idx_transactions_customer_time ON transactions (customer_id, transaction_time);")
    print(f"   Created idx_transactions_customer_time in {time.time() - start:.2f}s")
    ...
    conn.commit()
    print(f"   Added FK constraints in {time.time() - start:.2f}s")
...
def main():
    print("🚀 Seeding Docker PostgreSQL (Port 5433) with 10 MILLION rows...")
